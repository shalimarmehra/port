      ...
      const formattedPins = data.items.map((pin) => {
        ...
        return {
          ...
          title: decodeEntities(
            pin.title || pin.description || "Pinterest Pin",
          ),
...
async function fetchFromRSS(handle) {
  try {
    ...
    if (!response.ok) {
      console.error(
        "Pinterest RSS error status:",
        response.status,
        response.statusText,
      );
      return NextResponse.json(
        { error: `Failed to fetch Pinterest RSS for ${handle}` },
        { status: 500 },
      );
    }
    ...
    const channelTitleMatch = xmlText.match(
      /<channel>[\s\S]*?<title>([\s\S]*?)<\/title>/,
    );
    const channelDescriptionMatch = xmlText.match(
      /<channel>[\s\S]*?<description>([\s\S]*?)<\/description>/,
    );
    const channelLinkMatch = xmlText.match(
      /<channel>[\s\S]*?<link>([\s\S]*?)<\/link>/,
    );

    const channelTitle = channelTitleMatch
      ? channelTitleMatch[1].trim()
      : handle;
    const channelDescription = channelDescriptionMatch
      ? channelDescriptionMatch[1].trim()
      : "";
    const channelLink = channelLinkMatch
      ? channelLinkMatch[1].trim()
    ...
    return NextResponse.json(
      { error: `Internal server error for handle ${handle}` },
      { status: 500 },
    );
  }
}
