      ...
      setTimeout(() => {
        if (typeof window !== "undefined") {
          window.dispatchEvent(
            new CustomEvent("portfolio-view-change", { detail: saved }),
          );
        }
      }, 50);
    }
    ...
    <>
      ...
      <div
        ...
        className={`sticky top-[72px] z-40 w-full flex flex-col items-center justify-center transition-all duration-300 ${
          isSticky ? "py-3 bg-transparent" : "pt-16 pb-4 bg-transparent"
        }`}
      >
        {/* Title: Hides when sticky to save vertical space */}
        <h2
          className={`text-xs font-bold uppercase tracking-[0.2em] transition-all duration-300 overflow-hidden ${
            isSticky
